...
void read_and_print_registros_validos(ifstream &input, ofstream &output, int date_of_account,
    ...
    while (true) {
        ...
        date_of_account_leido = read_date(input); //* 01/04/2021
        if (input.eof())break;
        ignorar_coma(input);
        input >> char_code_leido >> int_code_leido; //* F3765
        ignorar_coma(input);
        //! SE IGNORA EL CANAL
        input.ignore(10000,','); //////Fextralife
        ignorar_coma(input);
        input >> follows_leido; //* 1734810
        ignorar_coma(input);
        input >> views_leido; //* 93036735
        ignorar_coma(input);
        input >> chat_leido; //* 222720
        ignorar_coma(input);
        ...
        if (date_of_account_leido == date_of_account and char_code_leido == char_code_stream
            ...
            if (date_of_account_leido != date_of_account) {
...
void registro_por_canal(ifstream &input, ofstream &output, int date_limite_inf, int date_limite_sup,
    ...
    while (true){
        ...
        if (date_limite_inf <= date_of_account and date_of_account <= date_limite_sup) {
            n_canal++; print_info_canal(output, n_canal); //* HEADER DE CADA CANAL
            ignorar_coma(input);
            input >> char_code_stream >> int_code_stream;
            ignorar_coma(input);
            generate_whitespaces(output, 6);
            //! IMPRESION DEL NOMBRE (lectura tmb btw) - CODIGO DE CANAL - FECHA DE CREACION
            ...
            ignorar_coma(input);
            //! IMPRESION DE TODO LO DEMAS
            output << setw(ANCHO_REPORTE_JUSTIFICADO/N_COLUMNS_POR_CANAL) << new_follows_of_month << setfill(' ');
            output << setw(ANCHO_REPORTE_JUSTIFICADO/N_COLUMNS_POR_CANAL) << new_views_of_month << setfill(' ');
            output << setw(ANCHO_REPORTE_JUSTIFICADO/N_COLUMNS_POR_CANAL) << max_ammount_of_chat << setfill(' ')<<endl;
            ...
            read_and_print_registros_validos(input, output, date_of_account,
                                        ...
                                        duracion_total_reproduccion, total_reproducciones,
                                        ingresos_por_publidad, ingresos_por_duracion);
            //? EN TEORIA AQUI DEBERIA LEER OTRO CANAL (PARA USARLO COMO PARAMETRO DE BUSQUEDA DE PASO)
            //? MI IDEA ES NO USAR .SEEKG() Y EL .CLEAR
        }
        else {
            input.ignore(10000,'\n');
        }

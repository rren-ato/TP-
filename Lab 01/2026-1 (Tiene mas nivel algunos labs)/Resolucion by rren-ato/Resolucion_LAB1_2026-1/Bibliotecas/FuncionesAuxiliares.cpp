...
void filter_of_lvl_pressure(double sistolic_pressure, double diastolic_pressure, int *cant_pac_normal_prs,
    ...
    else if (valid_interval_1 == true) {
        cout << "(Elevada)" << endl;
    }
    else if ((130 <= sistolic_pressure and sistolic_pressure <= 139) or 80 <= diastolic_pressure and diastolic_pressure<=89){
        cout << "(Hipertensión Nivel 1)" << endl;
    }
    else if ((140 <= sistolic_pressure) or 90 <= diastolic_pressure) {
        (*cant_pac_HP2_prs)++;
...
void reporte_parcial(int *cant_pac_normal_prs, int *cant_pac_HP2_prs, int *duracion_total,
    ...
    int DNI = 0, hora_entrada = 0, hora_salida = 0, duracion = 0;
    ...
    while (true) {
        ...
        if (hora_entrada > hora_salida) {
            duracion = hora_salida + (24*60*60) - hora_entrada;
        }
        else {
        ...
        if (*low_ranked_prs_ammount < 0 or middle_pressure < *low_ranked_prs_ammount) {
...
void reporte_final() {
    ...
    double low_ranked_prs_ammount = -1;
    ...
    while (not cin.eof()) {
        ...
        if (cin.eof()) break;
        print_line('=');
        // Impresion de la fecha y validacion de intervalos
        fecha = read_date();
        if (fecha < 20230405) {
            cin.ignore(10000, '\n');
            continue;
        }
        if (20230427 < fecha) break; //deberia ser otro continue pero las fechas estan ordenadas de menor a mayor
        cout << "FECHA: ";           // estan ordenadas de forma consecutiva
        print_date(fecha);
        ...
        cout << fixed << setprecision(4) << total_price_of_at << endl;
    }
}
